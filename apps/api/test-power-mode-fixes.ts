...
async function testPowerModeFixes() {
  ...
  try {
    ...
    console.log('📝 Response preview:', timeResult.messages[0].content.substring(0, 100) + '...\n');
    
    // Test 2: Complex research query (should use research agent)
    console.log('🔍 Test 2: Complex research query');
    const researchQuery = [new HumanMessage("Research the latest AI developments and analyze their impact")];
    ...
    console.log('📝 Response preview:', researchResult.messages[0].content.substring(0, 100) + '...\n');
    
    // Test 3: Multi-tool query (should use multiple tools)
    console.log('🔍 Test 3: Multi-tool query');
    const multiQuery = [new HumanMessage("What's the weather and time? Also search for local events")];
    ...
    console.log('📝 Response preview:', multiResult.messages[0].content.substring(0, 100) + '...\n');
