...
export class SimpleQueryHandler {
  private static simpleResponses: Record<string, SimpleResponse> = {
    ...
    'what can you do': {
      ...
      response: 'I can help with many things! 😊 I can answer questions, analyze documents, help with research, and much more. What would you like to know?',
      category: 'status'
    },
    'who are you': {
      query: 'who are you',
      response: 'I\'m Nurospace AI, your intelligent assistant! 🤖 I\'m here to help you with questions, analysis, and various tasks. How can I assist you today?',
      category: 'status'
    },
    'what is your name': {
      query: 'what is your name',
      response: 'My name is Nurospace AI! 🤖 I\'m your intelligent assistant, ready to help with whatever you need. What can I do for you today?',
      category: 'status'
    },
  ...
  static handleSimpleQuery(query: string): string | null {
    ...
    if (words.length <= 3) {
      ...
      if (this.simpleResponses[firstWord]) {
        console.log(`[SIMPLE QUERY] First word match response for: "${query}" (matched: "${firstWord}")`);
        return this.simpleResponses[firstWord].response;
      }
      
      // Check if it's a time-based greeting with additional words
      if (words.length === 2 && (firstWord === 'good' || firstWord === 'great')) {
        const secondWord = words[1];
        const timeGreeting = `${firstWord} ${secondWord}`;
        if (this.simpleResponses[timeGreeting]) {
          console.log(`[SIMPLE QUERY] Time greeting match response for: "${query}" (matched: "${timeGreeting}")`);
          return this.simpleResponses[timeGreeting].response;
        }
      }
      
      // Check for "how are you" variations
      if (words.length === 3 && firstWord === 'how' && words[1] === 'are' && words[2] === 'you') {
        console.log(`[SIMPLE QUERY] How are you match response for: "${query}"`);
      ...
      if (words.length === 3 && firstWord === 'i' && words[1] === 'don\'t' && words[2] === 'know') {
        console.log(`[SIMPLE QUERY] I don't know match response for: "${query}"`);
        return this.simpleResponses['i don\'t know'].response;
      }
      
      // Check for "i think so" variations
      if (words.length === 3 && firstWord === 'i' && words[1] === 'think' && words[2] === 'so') {
    ...
    for (const [key, responseData] of Object.entries(this.simpleResponses)) {
      if (normalizedQuery.startsWith(key + ' ') || normalizedQuery.endsWith(' ' + key)) {
        console.log(`[SIMPLE QUERY] Partial match response for: "${query}" (matched: "${key}")`);
  ...
  static getResponsesByCategory(category: string): SimpleResponse[] {
    return Object.values(this.simpleResponses).filter(response => response.category === category);
  }
  ...
  static addResponse(query: string, response: string, category: string): void {
    ...
    console.log(`[SIMPLE QUERY] Added new response: "${query}" -> "${response}" (${category})`);
  }
