...
export class MaxQueryDecomposer {
  ...
  async decomposeQuery(query: string, userId?: string): Promise<QueryDecomposition> {
    ...
    try {
      ...
      console.log("[MAX MODE] ✅ Generated", subQuestions.length, "sub-questions");
      ...
      const processingTime = this.estimateProcessingTime(subQuestions, complexity);
      console.log("[MAX MODE] ✅ Estimated processing time:", processingTime, "ms");
      ...
      console.log("[MAX MODE] ✅ Query decomposed successfully in", totalTime, "ms:", {
        intent,
      ...
      const totalTime = Date.now() - startTime;
      console.error("[MAX MODE] ❌ Query decomposition failed after", totalTime, "ms:", error);
  ...
  private async classifyIntent(query: string): Promise<string> {
    ...
    try {
      ...
      const intent = response.content?.toString().trim().toUpperCase() || 'RESEARCH';
      return intent;
    } catch (error) {
      console.warn("[MAX MODE] Intent classification failed, defaulting to RESEARCH");
      return 'RESEARCH';
    }
  }

  private async generateSubQuestions(query: string, intent: string): Promise<SubQuestion[]> {
    console.log("[MAX MODE] 🔍 Generating sub-questions for intent:", intent);
    
    // Check if LLM is properly initialized
    if (!this.llm || typeof this.llm.invoke !== 'function') {
      console.warn("[MAX MODE] ⚠️ LLM not properly initialized, using fallback sub-questions");
...
Respond with ONLY the JSON array:`;
    ...
    try {
      console.log("[MAX MODE] 🔍 Sending prompt to LLM for sub-question generation...");
      ...
      if (!response || !response.content) {
        console.warn("[MAX MODE] ⚠️ LLM returned empty response, using fallback");
        return this.createStructuredSubQuestions(query, intent);
      }
      
      const content = response.content.toString();
      console.log("[MAX MODE] 🔍 LLM response received, length:", content.length);
      ...
      try {
        ...
        console.log("[MAX MODE] ✅ JSON parsing successful, found", subQuestions.length, "sub-questions");
      } catch (parseError) {
        console.warn("[MAX MODE] ⚠️ JSON parsing failed:", parseError);
        console.log("[MAX MODE] 🔄 Creating structured sub-questions as fallback");
      ...
      console.log("[MAX MODE] ✅ Sub-questions generated successfully:", validatedSubQuestions.length);
      return validatedSubQuestions;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.warn("[MAX MODE] ⚠️ Sub-question generation failed:", errorMessage);
      console.log("[MAX MODE] 🔄 Using fallback structured sub-questions");
      return this.createStructuredSubQuestions(query, intent);
    }
  }

  private createStructuredSubQuestions(query: string, intent: string): SubQuestion[] {
    console.log("[MAX MODE] 🔄 Creating fallback structured sub-questions for intent:", intent);
    
    // Create context-aware sub-questions based on the query content
    const normalizedQuery = query.toLowerCase();
    
    // Check if this is an AI/job market analysis query
    if (normalizedQuery.includes('artificial intelligence') || normalizedQuery.includes('ai') || normalizedQuery.includes('job market')) {
      return [
        {
          id: 'sq_1',
          question: "What is the current state of artificial intelligence adoption in job markets?",
        ...
        {
          id: 'sq_2',
          question: "What are the emerging trends in AI's impact on employment and workforce?",
        ...
        {
          id: 'sq_3',
          question: "What strategies and recommendations exist for workforce adaptation to AI changes?",
    ...
    if (normalizedQuery.includes('compare') || normalizedQuery.includes('vs') || normalizedQuery.includes('difference')) {
      return [
        {
          id: 'sq_1',
          question: "What are the key characteristics of the first item being compared?",
          type: 'research' as const,
          priority: 1,
          dependencies: [],
          expectedTools: ['tavilySearch'],
        ...
        {
          id: 'sq_2',
          question: "What are the key characteristics of the second item being compared?",
          type: 'research' as const,
          priority: 2,
          dependencies: ['sq_1'],
          expectedTools: ['tavilySearch'],
        ...
        {
          id: 'sq_3',
          question: "What are the main differences and similarities between these items?",
    ...
    if (intent === 'SYNTHESIS' || normalizedQuery.includes('synthesize') || normalizedQuery.includes('combine') || normalizedQuery.includes('integrate')) {
      return [
        {
          id: 'sq_1',
          question: "What are the key components and sources of information for this topic?",
          type: 'research' as const,
          priority: 1,
          dependencies: [],
          expectedTools: ['tavilySearch'],
        ...
        {
          id: 'sq_2',
          question: "How do these different sources and components relate to each other?",
          type: 'analysis' as const,
          priority: 2,
          dependencies: ['sq_1'],
        ...
        {
          id: 'sq_3',
          question: "What is the integrated understanding and comprehensive view of this topic?",
          type: 'synthesis' as const,
          priority: 3,
          dependencies: ['sq_1', 'sq_2'],
          expectedTools: ['tavilySearch', 'retrieveRelevantChunks'],
  ...
  private determineExecutionOrder(subQuestions: SubQuestion[]): string[] {
    ...
    const sortedQuestions = [...subQuestions].sort((a, b) => a.priority - b.priority);
  ...
  private calculateComplexity(subQuestions: SubQuestion[]): 'low' | 'medium' | 'high' | 'critical' {
    const totalQuestions = subQuestions.length;
    const avgPriority = subQuestions.reduce((sum, q) => sum + q.priority, 0) / totalQuestions;
  ...
  private estimateProcessingTime(subQuestions: SubQuestion[], complexity: string): number {
    ...
    const totalTime = subQuestions.length * baseTimePerQuestion * complexityMultiplier[complexity as keyof typeof complexityMultiplier];
