...
import { maxQueryDecomposer, type QueryDecomposition } from "./max-query-decomposer";
import { maxExecutionOrchestrator, type ExecutionResult } from "./max-execution-orchestrator";
...
export class MaxModeSupervisor {
  ...
  async processQuery(query: string, userId?: string): Promise<MaxModeResponse> {
    ...
    try {
      ...
      const enhancedTools = await this.selectEnhancedTools(query, decomposition);
      ...
      const enhancedResponse = await this.enhanceResponse(query, executionResult);
      ...
      const recommendations = await this.generateRecommendations(query, qualityMetrics, decomposition);
  ...
  private async selectEnhancedTools(query: string, decomposition: QueryDecomposition): Promise<string[]> {
    ...
    return selectedTools.slice(0, MAX_MODE_CONFIG.toolSelection.maxToolsPerQuery);
  }
  ...
  private async enhanceResponse(query: string, executionResult: ExecutionResult): Promise<string> {
...
Enhanced Response:`;
    ...
    try {
      ...
      const enhancedResponse = response.content?.toString() || executionResult.finalResponse;
      
      if (enhancedResponse && enhancedResponse.length > executionResult.finalResponse.length) {
        console.log("[MAX MODE] ✅ Response enhancement successful, length increased from", 
          executionResult.finalResponse.length, "to", enhancedResponse.length);
        return enhancedResponse;
      } else {
        console.log("[MAX MODE] ⚠️ Response enhancement returned insufficient content, using fallback formatting");
  ...
  private formatResponseFallback(query: string, executionResult: ExecutionResult): string {
    ...
    if (originalResponse.includes('Search Results:')) {
      ...
      formattedResponse += originalResponse
    ...
    formattedResponse += `• **Steps Completed:** ${executionResult.steps.filter(s => s.status === 'completed').length}/${executionResult.steps.length}\n`;
    ...
    formattedResponse += `• Consider asking follow-up questions for specific details\n`;
  ...
  private assessQuality(executionResult: ExecutionResult): {
    ...
    const completedSteps = executionResult.steps.filter(s => s.status === 'completed');
    const totalSteps = executionResult.steps.length;
    
    if (totalSteps === 0) {
      return { accuracy: 0, completeness: 0, sourceAttribution: 0, confidence: 0 };
    }
    
    const accuracy = completedSteps.reduce((sum, step) => {
      if (step.result && Array.isArray(step.result)) {
        const stepAccuracy = step.result.reduce((acc, r) => acc + (r.confidence || 0), 0) / step.result.length;
  ...
  private async generateRecommendations(
    ...
    if (qualityMetrics.accuracy < 0.8) {
      recommendations.push("Consider rephrasing your question for more accurate results");
    }
    
    if (qualityMetrics.completeness < 0.8) {
      recommendations.push("Some analysis steps failed - try breaking down your question");
    }
    ...
    if (decomposition.estimatedComplexity === 'critical') {
      recommendations.push("This is a complex query - consider using multiple focused questions instead");
    }
    
    if (decomposition.subQuestions.length > 3) {
      recommendations.push("Consider focusing on the most important aspects of your question");
    }
    
    // Tool-specific recommendations
    if (decomposition.subQuestions.some(q => q.expectedTools.includes('retrieveRelevantChunks'))) {
      recommendations.push("Upload relevant documents for more accurate analysis");
    }
    
    if (decomposition.subQuestions.some(q => q.expectedTools.includes('tavilySearch'))) {
      recommendations.push("Consider asking for the most recent information available");
    }
  ...
  async integrateWithSemanticSupervisor(query: string, userId?: string): Promise<any> {
    try {
  ...
  private shouldUseMaxMode(query: string): boolean {
    ...
    const complexIndicators = [
      'compare', 'analyze', 'research', 'investigate', 'examine',
      'evaluate', 'assess', 'synthesize', 'verify', 'fact check',
    ...
    const hasMultipleParts = normalizedQuery.includes(' and ') || 
