...
export async function enhancedSemanticProcessing(query: string, userId?: string) {
  try {
    console.log("[EXAMPLE] 🔄 Enhanced semantic processing for:", query);
    
    // First, try MAX mode for complex queries
    const maxModeResult = await maxModeSupervisor.integrateWithSemanticSupervisor(query, userId);
    ...
    const normalResult = await semanticSupervisor([new HumanMessage(query)], { configurable: { userId } });
    
    return {
      mode: 'normal',
      response: normalResult.messages[0]?.content || "No response generated",
      confidence: 0.7, // Default confidence for normal mode
      quality: { accuracy: 0.7, completeness: 0.7, sourceAttribution: 0.5, confidence: 0.7 },
      recommendations: []
    };
...
export async function smartModeSelection(query: string, userId?: string) {
  try {
    ...
    if (shouldUseMaxMode) {
      ...
      const semanticSupervisor = createSemanticSupervisor(false);
      const result = await semanticSupervisor([new HumanMessage(query)], { configurable: { userId } });
      return result;
    }
...
export async function customMaxModeProcessing(query: string, userId?: string, customConfig?: any) {
  try {
...
export async function batchMaxModeProcessing(queries: string[], userId?: string) {
  try {
    console.log("[EXAMPLE] 📦 Batch processing with MAX mode:", queries.length, "queries");
    ...
    console.log(`[EXAMPLE] ✅ Batch processing completed: ${successCount}/${queries.length} successful`);
...
export async function monitoredMaxModeProcessing(query: string, userId?: string) {
  ...
  try {
    ...
    const endTime = Date.now();
    console.error(`[EXAMPLE] ❌ Monitored processing failed after ${endTime - startTime}ms:`, error);
    throw error;
  }
}

// Example 7: Integration with Chat Service
export async function maxModeChatIntegration(query: string, userId?: string, chatContext?: any) {
  try {
