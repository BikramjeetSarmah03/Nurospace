...
export class ProductionToolRegistry {
  ...
  private initializeRegistry(toolset: any[]) {
    ...
    for (const tool of toolset) {
      this.addTool(
        tool,
        toolMetadata.find((m) => m.name === tool.name),
      );
    }
    ...
    console.log(
      `[DEBUG] Production Tool Registry initialized with ${this.toolMap.size} tools in ${initTime}ms`,
    );
  }
  ...
  generateSystemMessage(): string {
    ...
    const categories = new Set(
      Array.from(this.metadata.values()).map((m) => m.category),
    );
  ...
  private generateMetadata(tool: any): ToolMetadata {
    ...
    let category: "research" | "analysis" | "execution" | "planning" =
      "planning";
    if (
    ...
    const keywords = [
      name,
      ...name.split(/(?=[A-Z])|_|-/).map((k: string) => k.toLowerCase()),
    ];
