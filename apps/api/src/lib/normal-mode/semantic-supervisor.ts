  ...
  for (const tool of tools) {
    try {
      ...
      if (tool.name === "getCurrentDateTime") {
        toolResult = await tool.invoke("Get current time and date", {
          configurable: { userId },
        });
        results.push(`🕐 **Current Time**: ${toolResult}`);
      } else if (tool.name === "getCurrentWeather") {
        toolResult = await tool.invoke("Get current weather information", {
          configurable: { userId },
        });
        results.push(`🌤️ **Weather**: ${toolResult}`);
      } else if (tool.name === "retrieveRelevantChunks") {
        const rawChunks = await tool.invoke(userQuery || "", {
          configurable: { userId },
        });
        ...
        if (rawChunks && rawChunks.length > 0) {
          ...
          results.push(
            "📄 **Document Analysis**: No relevant information found in your documents.",
          );
        }
      } else if (tool.name === "tavilySearch") {
        toolResult = await tool.invoke(userQuery || "", {
          configurable: { userId },
        });
...
export function createSemanticSupervisor(useFallback = false) {
  ...
  return async (
    ...
    try {
      ...
      const selection = await semanticRegistry.selectToolsSemantically(
        userQuery,
        undefined,
        3,
      );
          ...
          messages: [
            {
              role: "assistant",
              content:
                "I'm not sure how to help with that request. Could you please rephrase or provide more details?",
            },
          ],
          metadata: {
      ...
      const finalContent = await processToolResultsWithLLM(
        toolResult.content,
        userQuery,
      );
 ...
 */
export async function processToolResultsWithLLM(
  toolResults: string,
  userQuery: string,
): Promise<string> {
  try {
...
function handleSimpleQuery(query: string): string | null {
  ...
  if (
    ...
    normalizedQuery === "how are you doing"
  ) {
    return "I'm doing great, thank you! 😊 How can I assist you?";
  }

  if (
    normalizedQuery === "thanks" ||
    normalizedQuery === "thank you" ||
    normalizedQuery === "thx"
  ) {
  ...
  if (
    normalizedQuery === "bye" ||
    normalizedQuery === "goodbye" ||
    normalizedQuery === "see you"
  ) {
  ...
  if (
    normalizedQuery.includes("nice to meet you") ||
    normalizedQuery.includes("pleasure to meet you")
  ) {
  ...
  if (
    normalizedQuery.includes("have a good day") ||
    normalizedQuery.includes("have a nice day")
  ) {
