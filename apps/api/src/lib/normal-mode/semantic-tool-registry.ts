...
export class SemanticToolRegistry extends ProductionToolRegistry {
  ...
  private queryCache = new Map<
    string,
    { tools: any[]; confidence: number; timestamp: number }
  >();
  private toolPerformance = new Map<
    string,
    { successCount: number; totalUsage: number; lastUsed: number }
  >();
  private queryComplexityHistory = new Map<
    string,
    { keywordWorked: boolean; semanticNeeded: boolean; usageCount: number }
  >();
  ...
  public getInitializationStatus(): {
    ...
    return {
      ...
      error: this.initializationPromise
        ? undefined
        : "Initialization failed or not started",
    };
  }
    ...
    try {
      ...
      for (const toolConfig of TOOL_CONFIGS) {
        try {
          ...
          console.error(
            `[ERROR] Failed to generate embedding for ${toolConfig.name}:`,
            toolError,
          );
        }
      }
      ...
      console.log(
        `[DEBUG] Semantic embeddings generated for ${successCount}/${TOOL_CONFIGS.length} tools in ${Date.now() - startTime}ms`,
      );
      console.log(
        `[DEBUG] Tool embeddings map size: ${this.toolEmbeddings.size}`,
      );
  ...
  async selectToolsSemantically(
    ...
    console.log(
      `[DEBUG] Registry status: ready=${status.ready}, embeddings=${status.embeddingsCount}, error=${status.error || "none"}`,
    );
    ...
    console.log(
      "[DEBUG] Semantic predictions:",
      semanticPredictions.map(
        (p) => `${p.toolName}: ${(p.confidence * 100).toFixed(1)}%`,
      ),
    );
    console.log(
      "[DEBUG] Keyword predictions:",
      keywordPredictions.map(
        (p) => `${p.toolName}: ${(p.confidence * 100).toFixed(1)}%`,
      ),
    );
    ...
    console.log(
      ...
      ensembleResult.tools.map((t) => t.name),
    );
  ...
  private async getSemanticPredictions(query: string, maxTools: number) {
    ...
    console.log(
      `[DEBUG] this.toolEmbeddings size: ${this.toolEmbeddings.size}`,
    );
  ...
  private async getKeywordPredictions(query: string, maxTools: number) {
    ...
    if (queryLower.includes("@") || queryLower.match(/[a-f0-9-]{36}/)) {
      const tool = this.getAllTools().find(
    ...
    for (const toolConfig of TOOL_CONFIGS) {
      ...
      if (matchCount > 0) {
        ...
        if (tool) {
          ...
          if (
            !(
              toolConfig.name === "retrieveRelevantChunks" &&
              (queryLower.includes("@") || hasDocumentKeywords)
            )
          ) {
            predictions.push({
  ...
  private ensembleVoting(
    semanticPredictions: any[],
    keywordPredictions: any[],
    maxTools: number,
  ) {
    const toolScores = new Map<
      string,
      { score: number; tool: any; methods: string[] }
    >();
    ...
    if (keywordPredictions.length === 0) {
      console.log(
        "[DEBUG] No keyword predictions found, increasing semantic weight",
      );
      keywordWeight = 0.2;
    ...
    if (semanticPredictions.length > 1) {
      const semanticScores = semanticPredictions
        .map((p) => p.confidence)
        .sort((a, b) => b - a);
      const topTwo = semanticScores.slice(0, 2);
      if (topTwo.length === 2 && topTwo[0] - topTwo[1] < 0.05) {
        // Very close scores
        console.log(
          "[DEBUG] Semantic scores very close, relying more on keywords",
        );
        keywordWeight = 0.85;
        semanticWeight = 0.15;
      }
    ...
    console.log(
      `[DEBUG] Using weights: keyword=${(keywordWeight * 100).toFixed(0)}%, semantic=${(semanticWeight * 100).toFixed(0)}%`,
    );

    // Keyword vote with dynamic weight
    keywordPredictions.forEach((pred) => {
      const current = toolScores.get(pred.toolName) || {
        score: 0,
        tool: pred.tool,
        methods: [],
      };
    ...
    console.log(
      `[DEBUG] Adaptive threshold: ${(adaptiveThreshold * 100).toFixed(1)}% (based on top: ${(topScore * 100).toFixed(1)}%)`,
    );
    ...
    const reasonings = rankedTools.map(
      ([toolName, data]) =>
  ...
  private async semanticSelection(query: string, maxTools: number) {
    ...
    if (!this.embeddings) {
      ...
      return {
        ...
        confidenceScores: [],
        reasonings: [],
        selectionMethod: "no_semantic_model",
      };
    }
    ...
    try {
      ...
      console.log(
        `[DEBUG] Query embedding generated, length: ${queryEmbedding.length}`,
      );
      ...
      console.log(
        `[DEBUG] Calculating similarities with ${this.toolEmbeddings.size} tool embeddings...`,
      );
      ...
      for (const [toolName, toolEmbedding] of this.toolEmbeddings) {
        ...
        console.log(
          `[DEBUG] ${toolName} similarity: ${(similarity * 100).toFixed(1)}%`,
        );
      ...
      console.log(
        `[DEBUG] Found ${similarities.length} tools with similarities`,
      );
      ...
      similarities.sort((a, b) => {
        ...
        const aScore =
          a.similarity +
          (aPerf
            ? (aPerf.successCount / Math.max(aPerf.totalUsage, 1)) * 0.1
            : 0);
        const bScore =
          b.similarity +
          (bPerf
            ? (bPerf.successCount / Math.max(bPerf.totalUsage, 1)) * 0.1
            : 0);
      ...
      console.log(
        `[DEBUG] Selected tools after confidence filter (>0.3): ${selectedTools.length}`,
      );
      selectedTools.forEach((tool) => {
        console.log(
          `[DEBUG] Selected: ${tool.toolName} (similarity: ${(tool.similarity * 100).toFixed(1)}%)`,
        );
      });

      const tools = selectedTools.map((item) => item.tool);
      const confidenceScores = selectedTools.map((item) => item.similarity);
      const reasonings = selectedTools.map(
        (item) => `Semantic similarity: ${(item.similarity * 100).toFixed(1)}%`,
      );
  ...
  private async generateToolEmbedding(toolName: string, toolConfig: any) {
    ...
    try {
      ...
      console.log(
        `[DEBUG] Generating embedding for ${toolName} with text length: ${embeddingText.length}`,
      );

      const embedding = await this.embeddings.embedQuery(embeddingText);
      console.log(
        `[DEBUG] ✅ Generated embedding for ${toolName}, length: ${embedding.length}`,
      );

      this.toolEmbeddings.set(toolName, embedding);
      console.log(
        `[DEBUG] ✅ Stored embedding for ${toolName} in toolEmbeddings map`,
      );
    } catch (error) {
      console.error(
        `[ERROR] Failed to generate embedding for ${toolName}:`,
        error,
      );
