...
export default class ChatService {
  ...
  async newChat(c: Context, body: NewChatDto, user: User) {
    try {
      ...
      console.log(`[SIMPLE QUERY] Checking if "${userInput}" is a simple query...`);
      
      const simpleResponse = SimpleQueryHandler.handleSimpleQuery(userInput);
      if (simpleResponse) {
        console.log(`[SIMPLE QUERY] ✅ Detected simple query: "${userInput}" -> "${simpleResponse}"`);
        
        // Save the simple response
        await this.messageService.createMessage(chatId.id, "assistant", simpleResponse);
        ...
        console.log(`[SIMPLE QUERY] ❌ Not a simple query, proceeding with AI processing`);
      }
...
The right tools for each query will be made available to you seamlessly.`;
      ...
      return streamText(c, async (stream) => {
        ...
        try {
          ...
          if (requestedMode === "power") {
            selectedMode = "power";
          } else if (requestedMode === "max" || (requestedMode !== "normal" && autoDetectedMax)) {
            selectedMode = "max";
          } else {
            selectedMode = "normal";
          }
          
          console.log(`[MODE SELECTION] Frontend requested: "${requestedMode || 'auto'}", Auto-detected: ${autoDetectedMax}, Final decision: ${selectedMode.toUpperCase()}`);
          
          // ⚡ POWER MODE PROCESSING - Hybrid Supervisor Agent
          if (selectedMode === "power") {
            console.log(`[POWER MODE] 🚀 Frontend requested POWER mode, using Hybrid Supervisor Agent`);
            ...
            try {
              ...
              const hybridResult = await hybridAgent.processQuery(agentMessages, {
              ...
              if (isCancelled || c.req.raw.signal?.aborted) {
                console.log("[POWER MODE] Request was cancelled during processing");
                return;
              }
              ...
              // Stream the response
              if (!isCancelled) {
                const formattedResponse = this.formatPowerModeResponse(hybridResult);
              ...
              console.error("[POWER MODE] ❌ Processing failed, falling back to normal mode:", powerModeError);
          ...
          if (selectedMode === "max") {
            const modeReason = requestedMode === "max" ? "Frontend requested MAX mode" : "Auto-detected complex query";
            console.log(`[MAX MODE] 🚀 ${modeReason}, using MAX mode processing`);
        ...
        body.msg ?? "",
              ...
              if (isCancelled || c.req.raw.signal?.aborted) {
                console.log("[MAX MODE] Request was cancelled during processing");
                return;
              }
              ...
              if (!isCancelled) {
                const formattedResponse = this.formatMaxModeResponse(maxModeResult);
                await stream.write(formattedResponse);
              ...
              console.error("[MAX MODE] ❌ Processing failed, falling back to normal mode:", maxModeError);
              // Continue to normal processing as fallback
            }
          }
          ...
          const modeReason = requestedMode === "normal" ? "Frontend requested normal mode" : "Auto-selected normal mode";
          console.log(`[DEBUG] ${modeReason} - Using normal semantic processing`);
          ...
          if (finalResponse && finalResponse.content && !isCancelled) {
            ...
            if (responseContent.length > 10) {
              ...
              console.log("[DEBUG] Response too short, not saving:", responseContent.length);
            }
          }
          ...
          if (isCancelled || c.req.raw.signal?.aborted) {
            console.log("[DEBUG] Request was cancelled during error, not saving error message");
            return;
          }
  ...
  async cancelChat(chatSlug: string | undefined, user: User) {
    try {
      console.log("[DEBUG] Cancelling chat for user:", user.id, "chatSlug:", chatSlug);
        ...
        if (chat) {
          ...
          for (const response of allAIResponses) {
            if (response.content.length < 10) {
              ...
              console.log("[DEBUG] Removed incomplete AI response:", response.id);
            }
          }

          console.log("[DEBUG] Cleaned up incomplete responses for chat:", chat.id);
        }
      }
  ...
  private shouldUseMaxMode(query: string): boolean {
    ...
    const complexIndicators = [
      ...
      'comprehensive', 'detailed', 'thorough', 'in-depth', 'explain',
      'break down', 'compare and contrast', 'pros and cons'
    ];
    ...
    const isAnalysisQuestion = normalizedQuery.includes('how') || 
    ...
    const shouldUseMax = hasComplexIndicator || isLongQuery || hasMultipleParts || isAnalysisQuestion;
  ...
  private formatMaxModeResponse(maxModeResult: any): string {
    const { enhancedResponse, confidence, processingTime, qualityMetrics, recommendations } = maxModeResult;
    
    let formattedResponse = enhancedResponse;
    
    // Add quality metrics footer
    formattedResponse += `\n\n--- 🔍 ANALYSIS INSIGHTS ---
🎯 **Confidence Level**: ${(confidence * 100).toFixed(1)}%
⚡ **Processing Time**: ${processingTime}ms
📊 **Quality Metrics**:
   • Accuracy: ${(qualityMetrics.accuracy * 100).toFixed(1)}%
   • Completeness: ${(qualityMetrics.completeness * 100).toFixed(1)}%
   • Source Attribution: ${(qualityMetrics.sourceAttribution * 100).toFixed(1)}%`;
