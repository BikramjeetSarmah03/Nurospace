    ...
    func: async (input: string) => {
      ...
      switch (config.name) {
        ...
        case 'tavilySearch':
          ...
          try {
            const searchResults = await tavilySearch(input, tavilyApiKey, "basic", 5);
...
const dynamicTools = TOOL_CONFIGS
  .filter(config => config.name !== 'retrieveRelevantChunks') // Handle separately
...
export function getToolsByCategory(category: string) {
  ...
  return categoryTools.map(config => getToolByName(config.name)).filter(Boolean);
}
