...
function RouteComponent() {
  const { data: workflowsData, isLoading } = useSuspenseQuery({
    ...
    <div className="space-y-4 p-4 h-full">
      ...
      {isLoading ? (
        Array.from({ length: 4 }).map((_, idx) => (
          <Skeleton
            className="flex justify-between items-center bg-gray-200 p-2 w-full h-10"
            key={idx.toString()}
          >
            <Skeleton className="bg-gray-100 w-32 h-full" />
            <Skeleton className="bg-gray-100 w-4 h-full" />
          </Skeleton>
        ))
      ) : (
